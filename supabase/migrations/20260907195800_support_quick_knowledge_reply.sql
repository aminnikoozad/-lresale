-- Provide reliable deterministic replies for greetings and high-confidence approved FAQ matches.
-- Account-specific status questions remain routed to the Edge Function so they can use the authenticated customer's own data.

create or replace function private.support_quick_reply(p_body text)
returns text
language plpgsql
stable
security definer
...
as $$
declare
  q text := lower(trim(coalesce(p_body,'')));
  basic text;
  matched_answer text;
  matched_score numeric;
begin
  basic := private.support_basic_reply(p_body);
  if basic is not null then return basic; end if;

  if q ~ '(my pickup|pickup status|status.*pickup|my item|item status|status.*item|mon ramassage|statut.*collecte|mes articles|statut.*article)' then
    return null;
  end if;

  select s.approved_answer, s.score
  into matched_answer, matched_score
  from public.support_search_approved_knowledge(p_body, 1) s
  limit 1;

  if matched_answer is not null and coalesce(matched_score,0) >= 8 then
...
revoke all on function private.support_quick_reply(text) from public,anon,authenticated;
...
begin
  ...
  v_reply := private.support_quick_reply(p_message);
  if v_reply is not null then
    ...
    values(v_id,null,'ai','AI Assistant',v_reply,1,jsonb_build_object('provider','approved_quick_reply'));
...
begin
  ...
  if v_status='ai' then
    v_reply := private.support_quick_reply(p_body);
    if v_reply is not null then
      insert into public.support_messages(conversation_id,sender_id,sender_kind,sender_display_name,body,ai_confidence,metadata)
      values(p_conversation_id,null,'ai','AI Assistant',v_reply,1,jsonb_build_object('provider','approved_quick_reply'));
