-- Security/correctness hardening verified against the production schema on 2026-09-05.

-- Pickup request ownership, area/slot integrity and admin MFA-aware reads.
...
on public.collection_requests for insert to authenticated
with check (
...
drop policy if exists collection_requests_select_authenticated on public.collection_requests;
...
-- Return scheduler capacity when a request is cancelled.
...
-- RPC used by the Admin Pickup Inbox.
...
begin
  ...
  select status into v_old_status from public.collection_requests where id=p_request_id for update;
...
-- AI Trainer authorization uses the same MFA/AAL2 requirement as all privileged admin paths.
create or replace function public.support_admin_context()
returns table(
  user_id uuid, role text, display_name text, availability_status text,
  can_support boolean, ai_view boolean, ai_test boolean, ai_suggest_training boolean,
  ai_edit_training boolean, ai_approve_training boolean, ai_manage_behavior_rules boolean
)
language sql stable security definer set search_path = '' as $$
  select ar.user_id, ar.role, coalesce(sa.display_name,'Support Agent'), coalesce(sa.availability_status,'offline'),
    (ar.role='owner' or ar.can_support),
...
-- Human handoff is idempotent and does not spam admins when repeatedly requested.
create or replace function public.support_request_human(p_conversation_id uuid)
returns void language plpgsql security definer set search_path = '' as $$
declare v_priority text; v_status text; v_human_requested boolean;
begin
  if auth.uid() is null then raise exception 'Sign in required'; end if;
  ...
  if not v_human_requested and v_status not in ('waiting','human') then
    ...
    select sa.user_id,'human.requested',p_conversation_id,'New support request',
      'Customer requested a human support agent.',coalesce(v_priority,'normal')
    from public.support_admins sa where sa.active;
  end if;
end;
$$;

-- Sensitive resolved cases stay with humans if the customer replies again.
create or replace function public.support_send_customer_message(p_conversation_id uuid, p_body text)
returns uuid language plpgsql security definer set search_path = '' as $$
declare
  v_message uuid; v_status text; v_priority text; v_category text; v_assigned uuid; v_sensitive boolean;
begin
  ...
  if v_status='resolved' then
    ...
    select sa.user_id,'conversation.reopened',p_conversation_id,'Conversation reopened',
      case when v_sensitive then 'A sensitive customer conversation was reopened and is waiting for human support.'
           else 'A customer replied to a resolved support conversation.' end,
...
-- Support agents must actually own a conversation before finalizing it; Owner/Admin can override.
...
begin
  ...
  if v_role not in ('owner','admin') and v_assigned is distinct from auth.uid() then
    raise exception 'Take over the conversation before resolving it';
  end if;
...
begin
  ...
  select role into v_role from public.admin_roles where user_id=auth.uid();
  if v_role not in ('owner','admin') and v_assigned is distinct from auth.uid() then
    raise exception 'Take over the conversation before closing it';
  end if;
...
-- Assignment transitions are serialized and audited.
...
begin
  ...
  if not found then raise exception 'Conversation not found'; end if;
  select role into v_role from public.admin_roles where user_id=auth.uid();
  if v_current is not null and v_current<>auth.uid() and not (p_force and v_role in ('owner','admin')) then
  ...
  insert into public.support_assignments(conversation_id,admin_id,assigned_by,action)
  values(p_conversation_id,auth.uid(),auth.uid(),case when v_current is null then 'assigned' else 'taken_over' end);
  insert into public.support_audit_logs(actor_id,conversation_id,action,previous_value,new_value)
  values(auth.uid(),p_conversation_id,'conversation.taken_over',
    jsonb_build_object('assigned_to',v_current,'status',v_old_status),jsonb_build_object('assigned_to',auth.uid(),'status','human'));
end;
$$;
...
begin
  ...
  insert into public.support_assignments(conversation_id,admin_id,assigned_by,action)
  values(p_conversation_id,p_admin_id,auth.uid(),case when v_old is null then 'assigned' else 'taken_over' end);
  insert into public.support_notifications(admin_id,event_type,conversation_id,title,body,priority)
  select p_admin_id,'conversation.assigned',p_conversation_id,'Conversation assigned to you',coalesce(subject,'Support conversation'),priority
  from public.support_conversations where id=p_conversation_id;
  insert into public.support_audit_logs(actor_id,conversation_id,action,previous_value,new_value)
  values(auth.uid(),p_conversation_id,'conversation.assigned',
    jsonb_build_object('assigned_to',v_old,'status',v_old_status),jsonb_build_object('assigned_to',p_admin_id,'status','human'));
end;
$$;
...
begin
  if not private.is_support_admin() then raise exception 'Support access required'; end if;
  select priority,category,assigned_to,status into v_priority,v_category,v_assigned,v_old_status
  from public.support_conversations where id=p_conversation_id for update;
  if not found then raise exception 'Conversation not found'; end if;
  if v_assigned<>auth.uid() and not exists(select 1 from public.admin_roles where user_id=auth.uid() and role in ('owner','admin')) then
    raise exception 'Not assigned to you';
  end if;
  if v_priority in ('high','urgent') or v_category in ('Payment','Return') then
    raise exception 'Sensitive conversations must remain with human support';
  end if;
  update public.support_conversations
  set assigned_to=null,assigned_at=null,status='ai',ai_enabled=true,human_requested=false,waiting_since=null,updated_at=now()
  where id=p_conversation_id;
  insert into public.support_assignments(conversation_id,admin_id,assigned_by,action)
  values(p_conversation_id,null,auth.uid(),'returned_to_ai');
  insert into public.support_audit_logs(actor_id,conversation_id,action,previous_value,new_value)
  values(auth.uid(),p_conversation_id,'conversation.returned_to_ai',
    jsonb_build_object('assigned_to',v_assigned,'status',v_old_status),jsonb_build_object('assigned_to',null,'status','ai'));
end;
$$;

-- Not Helpful feedback is attached to the customer question that produced the AI answer.
create or replace function public.support_rate_ai_message(p_message_id uuid, p_helpful boolean)
returns void language plpgsql security definer set search_path = '' as $$
declare v_conv uuid; v_ai_created timestamptz; v_customer_message uuid; v_customer_question text; v_previous boolean;
begin
  if auth.uid() is null then raise exception 'Sign in required'; end if;
  ...
  if not p_helpful and v_previous is distinct from false then
    ...
    if v_customer_message is not null then
      update public.unknown_questions set training_priority=training_priority+1,updated_at=now() where message_id=v_customer_message;
      if not found then
...
-- Global support hours are Owner/Admin policy, not a Support Agent preference.
create or replace function public.support_update_business_hour(
  p_day smallint, p_enabled boolean, p_opens time without time zone,
  p_closes time without time zone, p_timezone text default 'America/Toronto'
)
returns void language plpgsql security definer set search_path = '' as $$
declare v_role text;
begin
  ...
  set enabled=excluded.enabled,opens_at=excluded.opens_at,closes_at=excluded.closes_at,
      timezone=excluded.timezone,updated_by=auth.uid(),updated_at=now();
end;
$$;

-- Atomic AI-turn claim prevents duplicate AI replies. Only the service-role JWT may call it.
alter table public.support_conversations
  add column if not exists ai_last_processed_message_id uuid references public.support_messages(id) on delete set null;
create index if not exists support_conversations_ai_last_processed_idx
  on public.support_conversations(ai_last_processed_message_id)
...
begin
  ...
  where c.id=p_conversation_id
    ...
    and exists(
      ...
      where m.id=p_message_id and m.conversation_id=c.id and m.sender_kind='customer' and m.sender_id=p_customer_id
    );
  return found;
end;
$$;

-- Database-side protection for commission tiers so a direct RPC cannot bypass UI validation.
create or replace function public.update_selling_rules(
  new_rules jsonb, change_reason text, new_effective_at timestamp with time zone default now()
)
returns integer language plpgsql security definer set search_path = '' as $$
declare
  previous_rules jsonb; next_version integer; minimum_value integer; pickup_value integer;
  tiers jsonb; tier_rec record; previous_max integer:=null; tier_index integer:=0;
begin
  ...
  loop
    ...
    if tier_rec.min_cents<0 or tier_rec.seller_bps<0 or tier_rec.platform_bps<0 or tier_rec.seller_bps+tier_rec.platform_bps<>10000 then
      raise exception 'Invalid commission tier';
    end if;
...
-- Remove PostgreSQL's default PUBLIC execute grant. Authenticated RPCs still enforce their own ownership/RBAC checks.
do $$
declare f record;
begin
  ...
  for f in
    ...
    where n.nspname='public' and p.proname not in ('claim_due_pickup_reminders','complete_pickup_reminder','support_claim_ai_turn')
  loop
    execute format('grant execute on function %s to authenticated',f.sig);
  end loop;
end $$;

grant execute on function public.catalog_items() to anon;
grant execute on function public.get_selling_rules() to anon;
grant execute on function public.get_shipping_policy() to anon;
grant execute on function public.is_username_available(text) to anon;
grant execute on function public.check_admin_login_rate_limit(text) to anon;
grant execute on function public.record_admin_login_failure(text) to anon;
grant execute on function public.clear_admin_login_failures(text) to anon;

revoke execute on function public.support_claim_ai_turn(uuid,uuid,uuid) from public,anon,authenticated;
revoke execute on function public.claim_due_pickup_reminders(integer) from public,anon,authenticated;
revoke execute on function public.complete_pickup_reminder(uuid,boolean,text) from public,anon,authenticated;
...
-- Foreign-key indexes used by support/AI workloads.
