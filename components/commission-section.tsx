...
export async function CommissionSection() {
  ...
  return (
    <>
      ...
      <AccountResaleInsights
        sellingPeriodDays={rules.sellingPeriodDays}
        minimumItemValue={formatCadFromCents(rules.minimumIndividualItemValueCents)}
        freePickupThreshold={formatCadFromCents(rules.pickupRules.freePickupThresholdCents)}
        lowValuePickupFee={formatCadFromCents(rules.pickupRules.lowValuePickupItemFeeCents)}
        storeCreditBonusBps={rules.storeCreditBonusBps}
      />
    </>
  );
}
