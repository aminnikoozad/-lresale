...
import { AlertCircle, CheckCircle2, CircleDollarSign, HeartHandshake, Package, RotateCcw, Shirt, ShoppingBag, Truck, Wallet } from "lucide-react";
...
type Request = { id:string; type:string; category:string; status:string; confirmationStatus:string; createdAt:string };
type ServiceArea = { id:string; city:string; pickupMode:string };
type PickupSlot = { id:string; serviceAreaId:string; label:string; remaining:number };
type Props = {
  name:string;
  message:string|null;
...
export function Dashboard({name,message,messageType,balance,totalEarned,items,requests,serviceAreas,pickupSlots}:Props){
  return <div className="dashboard">
    <section className="welcome"><div><p className="eyebrow dark">Customer dashboard</p><h1>Welcome, {name}.</h1><p>See your balance, follow every item and arrange your next collection.</p></div><div className="dash-actions"><RequestDialog type="bag" label="Request a Bag" icon={<Package/>} serviceAreas={serviceAreas} pickupSlots={pickupSlots}/><RequestDialog type="pickup" label="Request pickup" icon={<Truck/>} serviceAreas={serviceAreas} pickupSlots={pickupSlots}/></div></section>
    ...
    <section className="account-panel"><Tabs defaultValue="items"><TabsList><TabsTrigger value="items">My items</TabsTrigger><TabsTrigger value="requests">My requests</TabsTrigger><TabsTrigger value="payout">Payout</TabsTrigger></TabsList><TabsContent value="items"><div className="panel-title"><div><h2>Items we’re handling for you</h2><p>Follow each item from collection and inspection through listing, sale and payout.</p></div><RequestDialog type="pickup" label="Arrange collection" icon={<Truck/>} serviceAreas={serviceAreas} pickupSlots={pickupSlots}/></div>{items.length?<div className="item-table"><div className="table-head"><span>Item</span><span>Status</span><span>Sale value</span></div>{items.map((item)=><div className="table-row" key={item.id}><span>{item.name}</span><span><i className={`status ${item.status.toLowerCase()}`}/>{item.status}</span><strong>{item.price}</strong></div>)}</div>:<div className="empty-box"><Shirt/><h2>No items yet</h2><p>Your accepted items will appear here after collection and inspection.</p></div>}</TabsContent><TabsContent value="requests">{requests.length?<div className="request-list">{requests.map((request)=><article key={request.id}><div><b>{request.type}</b><span>{request.category} · {request.createdAt}</span></div><div><strong>{request.status}</strong><small>Confirmation: {request.confirmationStatus}</small></div></article>)}</div>:<div className="empty-box"><Package/><h2>No requests yet</h2><p>Order a Bag or arrange a pickup when your eligible items total at least $100.</p></div>}</TabsContent><TabsContent value="payout"><div className="payout-box"><Wallet/><div><h2>{balance} available</h2><p>Payout setup will become available after payment verification is connected.</p></div><Button disabled>Set up payout</Button></div></TabsContent></Tabs></section>
...
function RequestDialog({label,icon,type,serviceAreas,pickupSlots}:{label:string;icon:React.ReactNode;type:"bag"|"pickup";serviceAreas:ServiceArea[];pickupSlots:PickupSlot[]}){
    ...
    <DialogContent className="request-dialog">
      ...
      <form className="request-form" action={createCollectionRequest}>
        <div className="request-form-scroll">
          ...
          <label>Pickup city<select name="service_area_id" value={serviceAreaId} onChange={event=>setServiceAreaId(event.target.value)} required><option value="" disabled>Select a city</option>{serviceAreas.map((area)=><option key={area.id} value={area.id}>{area.city}{area.pickupMode==="free"?" — Free pickup":" — Subject to review"}</option>)}</select></label>
          <label>Available pickup time<select name="pickup_slot_id" required defaultValue=""><option value="" disabled>Select an available time</option>{availableSlots.map((slot)=><option key={slot.id} value={slot.id}>{slot.label} · {slot.remaining} spots left</option>)}</select></label>
          ...
          <div className="hold-card"><Truck/><div><b>Free pickup — no deposit or card hold</b><p>Eligible pickups have no upfront fee. We’ll ask you to confirm before adding the pickup to the driver’s route.</p></div></div>
          ...
          <label>Approximate number of items<Input name="item_count" required type="number" min="1" max="500" step="1" inputMode="numeric" placeholder="For example: 12"/></label>
          <label>Brands (optional)<Input name="brands" maxLength={500} placeholder="For example: Aritzia, Nike, Levi’s"/></label>
          ...
          <label className="check"><input name="policy_accepted" value="accepted" required type="checkbox"/> I accept the selling period and commission rates.</label>
          <label className="check"><input name="pickup_policy_accepted" value="accepted" required type="checkbox"/> <span>I accept the <Link href="/pickup-policy" target="_blank">Pickup &amp; Missed Pickup Policy</Link>.</span></label>
        </div>
        <div className="request-form-footer"><Button type="submit" disabled={!availableSlots.length}>Submit collection request</Button><small className="payment-note">Submitting a request does not guarantee free pickup approval.</small></div>
      </form>
    </DialogContent>
  </Dialog>;
}
