...
export const metadata: Metadata = {
  ...
  description: "See how REWEAR pickup, inspection, pricing, commission, processing fees and seller earnings work before you send anything.",
};

export default async function SellWithRewearPage() {
  const supabase = createPublicClient();
  const rules = await loadSellingRules(supabase);
  const processingFee = formatCadFromCents(rules.pickupRules.processingFeeCents);
  ...
  return (
    <main className="seller-guide-shell">
      ...
      <section className="guide-section guide-fees" id="fees">
        <div className="guide-heading"><p className="eyebrow dark">Fees & earnings</p><h2>No surprise charges.</h2><p>Fees are shown before you submit a new collection request. New batch fees are recorded on that request, so future rule changes do not rewrite the fee on an older batch.</p></div>
        <div className="fee-cards">
          <article><span>Processing</span><strong>{processingFee}</strong><p>Once per new batch/pickup. It is intended to be deducted from seller earnings when settlement is available, rather than charged to a card upfront.</p></article>
          <article><span>REWEAR Bag</span><strong>{bagFee}</strong><p>Only when you request a REWEAR Bag. Use your own bag or box and there is no Bag fee.</p></article>
          <article><span>Pickup</span><strong>{freePickupThreshold}+ = free</strong><p>Below {freePickupThreshold}, standard pickup is {lowValueFee} per item. This covers transportation for smaller collections.</p></article>
        </div>
        ...
        <div className="calculator-wrap">
          <div><h3>Estimate your earnings</h3><p>This is an estimate, not a payout promise. The batch processing fee is shown separately because it is charged once per batch rather than once per item.</p></div>
          <SellerEarningsCalculator tiers={rules.commissionTiers} processingFeeCents={rules.pickupRules.processingFeeCents} rewearBagFeeCents={rules.pickupRules.rewearBagFeeCents} minimumItemValueCents={rules.minimumIndividualItemValueCents} />
        </div>
      </section>
      ...
      <section className="guide-cta"><div><p className="eyebrow">Ready?</p><h2>Know the rules before the pickup.</h2><p>Your account shows the pickup fee, processing fee and REWEAR Bag fee that apply to a new batch before you submit it.</p></div><Link href="/account">Arrange collection <ArrowRight /></Link></section>
