...
export default async function PickupPolicyPage() {
  ...
  return <main className="pickup-policy-page">
    ...
    <article className="legal-policy">
      <div className="policy-hero"><p className="eyebrow dark">Customer policy</p><h1>Pickup &amp; Intake Fee Policy</h1><p>Pickup and the single batch service fee are shown before a new collection request is submitted.</p><small>Updated September 24, 2026</small></div>
      ...
      <section><h2>{serviceFee} Batch Service Fee</h2><p>Each new collection batch records one <strong>{serviceFee}</strong> service fee, not one processing fee plus another Bag fee. The same {serviceFee} total applies whether you request a REWEAR Bag or use your own suitable bag/box.</p><p>The fee is intended to be deducted from seller earnings when settlement is available rather than charged to a payment card upfront. It is snapshotted on the batch when the request is created, so a later policy change does not rewrite the fee on an older batch.</p></section>
